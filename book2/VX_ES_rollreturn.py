...
pnl = np.sum((positions.shift().values) * (y - y.shift()) - onewaytcost * np.abs(
    positions.values * y - (positions.values * y).shift()), axis=1)  # daily P&L of the strategy
...
print('APR=%f Sharpe=%f' % (np.prod(1 + ret[idx[500:]]) ** (252 / len(ret[idx[500:]])) - 1,
                            np.sqrt(252) * np.mean(ret[idx[500:]]) / np.std(ret[idx[500:]])))
